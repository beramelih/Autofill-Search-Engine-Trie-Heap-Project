...
Java 17 or newer
Apache Maven (recommended) or javac / java on the classpath
Compile and run
...
Walks the trie to the node for that prefix (or returns no matches if the prefix is absent).
Collects every dictionary word starting with that prefix.
Ranks matches by frequency with a max-heap and shows the top 10.
...
Search field — type any prefix; the list updates on every keystroke.
Ranked suggestions — words that start with your prefix, highest frequency first (up to 10).
Click a suggestion (or press Enter in the list) — fills the search field; typing is never overwritten by list refresh.
Add word — insert a custom word and frequency into the trie.
Load samples — clear and reload ~50 demo words.
...
Assignment checklist
 ...
 Trie with insert and prefix search
 Frequency-based ranking
 PriorityQueue (max-heap) central to ranking
 Swing desktop GUI
 Sample dictionary (~50 words)
 Prefix autocomplete for any typed prefix
 README with compile/run and concept explanations
